...
def GiRaFFE_Higher_Order():
    ...
    alphau0 = gri.register_gridfunctions("AUX","alphau0_dD")
    ...
    for i in range(DIM):
        Stilde_rhs[i] = thirdterm[i]
        for j in range(DIM):
            Stilde_rhs[i] += -alpsqrtgam * TEMUD_dD[j][i][j]
            for k in range(DIM):
                Stilde_rhs[i] += -gammaDD[i][k]*TEMUU[k][j]*alpsqrtgam_dD[j]
