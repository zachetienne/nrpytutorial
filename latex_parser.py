...
from sympy import var, sqrt, expand, simplify
import re
        ...
        for idx in set(idx_lst):
            ...
            if count > 1:
                ...
                if bound_count >= loop_count:
                    expr = 'sum(%s for %s in range(%d))' % (expr.replace(idx, lp_idx), lp_idx, dimension)
...
def parse(sentence, tensor=None, namespace=None, simplify_exprs=True):
    """ Convert LaTeX Sentence to SymPy Expression
