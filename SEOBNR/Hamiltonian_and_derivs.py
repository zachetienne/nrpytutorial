...
def output_second_derivs():
    ...
    with open("numpy_expressions.py", "w") as file:
        file.write("import numpy as np\n")
        file.write("def compute_dHdq(m1, m2, eta, x, y, z, px, py, pz, s1x, s1y, s1z, s2x, s2y, s2z, KK, c0k2, c1k2, c0k3, c1k3, c0k4, c1k4, c2k4, c0k5, c1k5, c2k5):\n")
        for i in range(len(lr)-1):
            file.write("\t" + lr[i].lhs + " = " + str(lr[i].rhs).replace("Abs(", "np.abs(").replace("Rational(",
