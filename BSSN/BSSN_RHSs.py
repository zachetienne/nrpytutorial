...
def bssn_metric():
    ...
    global gammabarDD = ixp.zerorank2() # Needed as global for Psi4.py
    for i in range(DIM):
        for j in range(DIM):
            gammabarDD[i][j] = hDD[i][j]*rfm.ReDD[i][j] + rfm.ghatDD[i][j]
    global gammabarUU, dummydet = ixp.symm_matrix_inverter3x3(gammabarDD) # Needed as global for Psi4.py
    ...
    global GammabarUDD = ixp.zerorank3() # Needed as global for Psi4.py
    for i in range(DIM):
        for k in range(DIM):
            for l in range(DIM):
                for m in range(DIM):
    ...
    global detgammabar = detgbar_over_detghat * rfm.detgammahat # Needed as global for Psi4.py
